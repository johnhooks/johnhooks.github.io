...
_Note: Any name or variable that need to be provided is wrapped in curly brackets,
example:_ `{user_name}`
...
Databases are not automatically created when restoring from a dump. Before restoring with
`psql` create the new database from `template0`. Also all users who own own or have
permissions on objects in the dumped database must already exist before restoring.
Otherwise it will not be restored with the same ownerships or permissions.
