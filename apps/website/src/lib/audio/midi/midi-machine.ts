...
export const midiMachine = createMachine<Context, MidiEvent, MidiTypestate>(
    ...
    states: {
      initial: {
        invoke: {
          ...
          src: () => {
            if (!browser) return Promise.reject(Error("Not in browser"));
  ...
  },
  {
    actions: {
      ...
      logNewConnection: (context, event) => {
        if (event.type === "OPEN") {
      ...
      logLostConnection: (context, event) => {
        if (event.type === "CLOSE") {
          const input = context.access.inputs.get(event.id);
          if (!input) return;
